...
func TestProber_getHttpChecker(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			want: &checkers.HTTP{
				Config: &checkers.HTTPConfig{
					...
					Method:     http.MethodGet,
					Headers:    http.Header{"Foo": {"bar"}, "User-Agent": {"http"}},
					StatusCode: 200,
					Client:     &http.Client{Timeout: time.Second},
					Timeout:    time.Second,
				},
		...
		},
		{
			...
			want: &checkers.HTTP{
				Config: &checkers.HTTPConfig{
					...
					Method:     http.MethodGet,
					Headers:    http.Header{"User-Agent": {"http"}},
					StatusCode: 204,
					Client:     &http.Client{Timeout: time.Second},
					Timeout:    time.Second,
				},
		...
		{
			name: "Happy path: a configured user agent is not overridden",
			fields: fields{
				Name:  "http",
				Probe: Probe{HttpGet: &HttpProbe{Headers: map[string]string{"User-Agent": "custom-agent"}}},
			},
			want: &checkers.HTTP{
				Config: &checkers.HTTPConfig{
					URL: &url.URL{
					...
					Method:     http.MethodGet,
					Headers:    http.Header{"User-Agent": {"custom-agent"}},
					StatusCode: 200,
					Client:     &http.Client{Timeout: time.Second},
					Timeout:    time.Second,
				},
