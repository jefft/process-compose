...
func (p *Probe) ValidateAndSetDefaults() {
	...
	// Poll fast until the process first responds. FailureThreshold is expressed
	// in PeriodSeconds units (see Prober.failureThresholdFor), so this does not
	// shorten the wall-clock grace given to a slow-starting process.
